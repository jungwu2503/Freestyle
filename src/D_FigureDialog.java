...
public class D_FigureDialog extends JDialog {
	static class DialogPanel extends JPanel implements ActionListener{
		...
		DialogPanel(JDialog dialog, D_DrawerView view) {
			this.view = view;
			this.dialog = dialog;
			this.setLayout(null);
		...
		private void onOk() {
			...
			D_Figure newFigure = null;
			if (selection.equals("Box")) {
				newFigure = new D_Box(Color.black,x1,y1,x2,y2);
				newFigure.setPopup(view.boxPopup());
			} else if (selection.equals("Line")) {
				newFigure = new D_Line(Color.black,x1,y1,x2,y2);	
	...
	D_FigureDialog(String title, D_DrawerView view) {
